module "iam_role_aws_load_balancer_controller" {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:${local.k8s_service_account_system_namespace}:aws-load-balancer-controller"]
  depends_on                    = [module.iam_policy_aws_load_balancer_controller]
}
...
resource "local_file" "aws_load_balancer_controller" {
  filename = "values/aws_load_balancer_controller.yml"
  content = yamlencode(merge({
    ...
    serviceAccount = {
      ...
      annotations = {
        "eks.amazonaws.com/role-arn" = module.iam_role_aws_load_balancer_controller.iam_role_arn
      }
    }
...
resource "helm_release" "aws_load_balancer_controller" {
  ...
  values = [local_file.aws_load_balancer_controller.content]
}